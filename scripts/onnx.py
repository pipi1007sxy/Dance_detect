...
def check_onnx_model_exists(output_name):
    if os.path.exists(output_name):
        print(f"ONNX model '{output_name}' has been successfully exported.")
        # （可选）验证ONNX模型的结构
        onnx_model = onnx.load(output_name)
        onnx.checker.check_model(onnx_model)
        print("ONNX model structure is valid.")
    else:
        print(f"ONNX model '{output_name}' does not exist. Export failed.")
...
if __name__ == '__main__':
    ...
    check_onnx_model_exists(args.output_name)